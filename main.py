...
from utils import plot_loss_acc, get_train_valid_loader, get_test_loader, mixup
...
def main(args):
    ...
    train_loader, valid_loader = get_train_valid_loader(args.dataset_dir, args.batch_size, args.mixup, args.seed, save_images=args.save_images)
    if args.test:
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, labels in train_loader:
            ...
            if args.mixup:
                ...
                # equals = top_class.eq(labels).sum().item()
            else:
                _, top_class = logits.topk(1, dim=1)

            equals = top_class == labels.cuda().view(*top_class.shape) # added
